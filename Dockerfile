# ─────────────────────────────────────────────────────────────────────────────
# Hungarian Financial Regulation MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t hungarian-financial-regulation-mcp .
# Run:    docker run --rm -p 3000:3000 hungarian-financial-regulation-mcp
#
# The image bakes /app/data/mnb.db at build time. Override MNB_DB_PATH
...
ENV MNB_DB_PATH=/app/data/mnb.db
...
# Bake the pre-built database into the image so /app/data/mnb.db resolves
# at runtime without a bind mount.
#
# The explicit `data/mnb.db` reference is required — `.github/workflows/
...
# step — it `gh release download`s `database.db.gz` and gunzips to that
# path. We then COPY it into the image at /app/data/mnb.db (MNB_DB_PATH).
COPY data/database.db data/mnb.db
